...
class steamauthOOP
{
	private $settings = array(
		"apikey" => "", // Get yours from http://steamcommunity.com/dev/apikey
	...
	function __construct()
	{
		...
		{
			die("<b>SteamAuthOOP:</b> Please supply a valid API-Key!");
		}

		if ($this->settings["loginpage"] == "")
		{
			$this->settings["loginpage"] = (!empty($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']; // Code (c) 2010 ichimonai.com, released under MIT-License
		}
	...
	/**
	 * Logout of website and unset session
	 */
	function logout()
	{
		if (!$this->loggedIn())
		...
		if (!isset($_SESSION[0]))
		{
			session_destroy(); // End the session if theres no more data in it
		}

		if ($this->settings["logoutpage"] != "")
		{
			header("Location: " . $this->settings["logoutpage"]); // If the logout-page is set, go there
		}
	...
	/**
	 * Reload the user's data from Steam (could be used after a user changes their
	 * username)
	 */
	function forceReload()
	{
		...
		{
			return false; // User is not logged in, nothing to reload
		}
		...
		foreach ($_SESSION["steamdata"] as $key => $value)
		{
			$this->{$key} = $value; // Make user-data accessable through $steam->var
		}
